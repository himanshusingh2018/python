...
2: RefSeq GTF file: 'hg38.refGene_noAltChr.txt'
   clean the hg38.refGene file as 'hg38.refGene_noAltChr.txt'
   "sed -n '/_alt/!p' hg38.refGene.txt >hg38.refGene_noAltChr.txt"
   file with below header line
...
class Expression_bedgraph:
    ...
    def __init__(self):
        '''Read File'''
    def induced_genes(self,expression_file,coordinate_file):
        os.chdir(cwd)
        ...
        induced_gene_exp_data = expression[(expression.DESeq2_log2FoldChange > 0)]
        #writing induced_genes
        induced_gene_exp_data.to_csv('./output/exp_bedgraph/k562_IFNa_hg38.induced_genes',sep="\t",header=False,index=False)
        ...
        refgene_data = pd.read_csv('input_data/'+self.coordinate_file,sep="\t",header=None,names=refgene_col)
        ...
        exp_bedgraph.to_csv('output/exp_bedgraph/k562_IFNa_'+self.expression_file[60:]+'hg19_refGene.bedgraph',sep=" ",header=False,index=False)
        print(" Expression Bedgraph file is generated:\n\toutput/exp_bedgraph/k562_IFNa_"+self.expression_file[60:]+'_hg19_refGene.bedgraph',"\n")
...
class Extract_induced_genes_coordinates:
    ...
    def induced_genes(self,coordinatefile,genelist):
        ...
        induced_gene_data = pd.read_csv('output/exp_bedgraph/'+self.genelist,sep="\t",header=0)
...
class Tf_peak_tss_binding:
    ...
    def tf_peak_tss_cluster(self,filename,tf,object,filterscore):
        ...
        tf_peak_file = pd.read_csv('input_data/'+self.filename,sep="\t",names=tf_peak_cols,header=None)        #macs2 $1,$2,$3,$5
        #tf_peak_file['chr'] = 'chr' + tf_peak_file['chr'].astype(str)
        filter = tf_peak_file['score'] > self.filterscore
        #print(tf_peak_file.head(5));exit(0)
        ...
        #print(tf_df);print(non_tf_df);exit(0)
...
genome_annotation_refseq_gtf = 'hg19.refGene_noAltChr.txt'
rna_expression_file = 'RNAseq_K562_DESeq2_EdgeR_pval_and_norm_count_log2.txt'
tf_name = 'STAT1'
tf_chiqseq_bedfile = 'STAT1_IFNa_30min_ENCFF290HTJ.bed'
...
expression_bedgraph.induced_genes(rna_expression_file,genome_annotation_refseq_gtf)
#expression_bedgraph.induced_genes('RNAseq_K562_DESeq2_EdgeR_pval_and_norm_count_log2.txt','hg19.refGene_noAltChr.txt')

extract_induced_genes=Extract_induced_genes_coordinates()
induced_genes_tss = extract_induced_genes.induced_genes('hg19.refGene_noAltChr.txt','k562_IFNa_hg38.induced_genes')
...
tf_peak_tss_binding.tf_peak_tss_cluster(tf_chiqseq_bedfile,tf_name,induced_genes_tss,filterscore=0)
#tf_peak_tss_binding.tf_peak_tss_cluster('STAT1_IFNa_30min_ENCFF290HTJ.bed','STAT1_IFNa_30min',induced_genes_tss,filterscore=0)
